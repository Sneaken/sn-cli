...
module.exports = class Generator {
  constructor (context, {
    ...
    this.files = Object.keys(files).length
      // when execute `sneaken add/invoke`, only created/modified files are written to disk
    ...
    const cliService = plugins.find(p => p.id === '@sneaken/cli-service')
  ...
  extractConfigFiles (extractAll, checkExisting) {
    ...
    if (extractAll) {
      ...
      if (!process.env.VUE_CLI_TEST) {
        // by default, always extract sneaken.config.js
        extract('sneaken')
      }
  ...
  sortPkg () {
    ...
    debug('sneaken:cli-pkg')(this.pkg)
  }
  ...
  async resolveFiles () {
    ...
    debug('sneaken:cli-files')(this.files)
  }
