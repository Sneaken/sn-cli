...
const { runTransformation } = require('sneaken-codemod')
const stringifyJS = require('./util/stringifyJS')
const ConfigTransform = require('./ConfigTransform')
const { semver, error, getPluginLink, toShortPluginId, loadModule } = require('@sneaken/cli-shared-utils')
...
class GeneratorAPI {
  ...
  constructor (id, generator, options, rootOptions) {
    ...
    this.pluginsData = generator.plugins
      .filter(({ id }) => id !== `@sneaken/cli-service`)
      .map(({ id }) => ({
  ...
  assertCliVersion (range) {
    ...
    throw new Error(
      `Require global @sneaken/cli "${range}", but was invoked by "${this.cliVersion}".`
    )
  }
  ...
  get cliServiceVersion () {
    ...
    const servicePkg = loadModule(
      '@sneaken/cli-service/package.json',
  ...
  assertCliServiceVersion (range) {
    ...
    throw new Error(
      `Require @sneaken/cli-service "${range}", but was loaded with "${this.cliServiceVersion}".`
    )
  }
   ...
   *
   * @param {string} id - Plugin id, can omit the (@sneaken/|sneaken-|@scope/sneaken)-cli-plugin- prefix
  ...
  addConfigTransform (key, options) {
    ...
    if (
      ...
      if (hasReserved) {
        const { warn } = require('@sneaken/cli-shared-utils')
  ...
  /**
   * Run codemod on a script file or the script part of a .sneaken file
