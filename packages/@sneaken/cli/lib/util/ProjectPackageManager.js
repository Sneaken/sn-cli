...
const isTestOrDebug = process.env.SN_CLI_TEST || process.env.SN_CLI_DEBUG
...
class PackageManager {
    ...
    if (this.bin === 'npm') {
      ...
      if (semver.lt(npmVersion, MIN_SUPPORTED_NPM_VERSION)) {
        throw new Error(
          ...
          'It does not support some core functionalities of SNEAKEN CLI.\n' +
  ...
  async getRegistry (scope) {
    ...
    let registry = registries.taobao
    if (args.registry) {
  ...
  async upgrade (packageName) {
    ...
    for (const packname of packageName.split(' ')) {
      ...
      if (
        isTestOrDebug &&
        (packname === '@sneaken/cli-service' || isOfficialPlugin(resolvePluginId(realname)))
      ) {
